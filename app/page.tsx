...
export default function HomePage() {
    ...
    async (pageNum: number, append = false) => {
      try {
        ...
        if (mode === "explore") {
          ...
          url = `/api/search?query=${encodeURIComponent(searchQuery)}&page=${pageNum}`
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !isFetchingMore && !isLoading && page < totalPages) {
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
              {movies.length > 0 ? (
                ...
                <div className="col-span-full text-center py-12">
                  <p className="text-muted-foreground">Tidak ada film ditemukan</p>
                </div>
              )}
            </div>
